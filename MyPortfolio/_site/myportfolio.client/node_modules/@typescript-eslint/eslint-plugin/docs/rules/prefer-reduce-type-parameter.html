...
<p>It’s common to call <code class="language-plaintext highlighter-rouge">Array#reduce</code> with a generic type, such as an array or object, as the initial value.
Since these values are empty, their types are not usable:</p>

<ul>
  <li><code class="language-plaintext highlighter-rouge">[]</code> has type <code class="language-plaintext highlighter-rouge">never[]</code>, which can’t have items pushed into it as nothing is type <code class="language-plaintext highlighter-rouge">never</code></li>
  <li><code class="language-plaintext highlighter-rouge">{}</code> has type <code class="language-plaintext highlighter-rouge">{}</code>, which doesn’t have an index signature and so can’t have properties added to it</li>
</ul>
...
While this will work, it’s not the most optimal solution as type assertions have subtle effects on the underlying types that can allow bugs to slip in.</p>
